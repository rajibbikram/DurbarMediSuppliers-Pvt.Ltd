...
const OfferItemManagement = () => {
  ...
  const handleDelete = async (itemId) => {
    if (!window.confirm('Are you sure you want to delete this offer item?')) {
      return;
    }

    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`http://localhost:5000/api/offer-items/${itemId}`, {
        method: 'DELETE',
        headers: {
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center bg-blue-600 text-white px-4 py-2 rounded-xl hover:bg-blue-700 transition-colors font-semibold"
            >
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:border-blue-500"
              />
            </div>
          </div>
        </div>
            ...
            <div className="text-center py-12 bg-gradient-to-br from-gray-50 to-blue-50 rounded-xl">
              <div className="flex flex-col items-center">
                ...
                <p className="text-gray-600 mb-4">
                  {searchTerm ? 'No offer items found matching your search' : 'No offer items yet'}
                </p>
                {!searchTerm && (
                  <button
                    onClick={() => navigate('/admin/offer-items/add')}
                    className="flex items-center bg-blue-600 text-white px-6 py-3 rounded-xl hover:bg-blue-700 transition-colors font-semibold"
                  >
                    <FaPlus className="mr-2" />
                    ...
                    <tr key={item._id} className="border-b border-gray-100 hover:bg-gradient-to-r from-blue-50 to-transparent transition-colors">
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center space-x-2">
                          <button
                            onClick={() => navigate(`/admin/offer-items/edit/${item._id}`)}
